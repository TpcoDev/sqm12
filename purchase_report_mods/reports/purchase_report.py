...
class PurchaseOrderLine(models.Model):
    ...
    def write(self, vals):
        if 'x_studio_comentarios' in vals:
            ...
            vals.update({'x_studio_hist_comentarios': vals.get('x_studio_comentarios', '') + "\n\n" + datetime.datetime.strptime(fields.datetime.now(tz = pytz.timezone(self.env.user.partner_id.tz)), '%Y-%m-%d %H:%M:%S').strftime('%d-%m-%Y %H:%M:%S') + ": " + record.x_studio_hist_comentarios})
